function randomLocationGenerator(numPoints) {
  ...
  that.create70Coords = H.util.Coroutine.create(
    'iterator',
  ...
  that.points = [];
  that.addCount  = 0;
  that.group = new H.map.Group();

  // Define the a coroutine - add 70 points.
  that.add70MarkersTogroup = H.util.Coroutine.create(
    'adder',
    function (scope, context) {
      var j,
        coord,
      ...
      for (j = 0; j  < 70; j += 1) {
        ...
        scope.group.addObject(new H.map.Marker(
          coord
        ));
      }
    },
  ...
  that.createCoordinates = function (callback) {
    var i,
      ...
      coroutineObserver = function (context, key, value, oldValue) {
        if (value === H.util.Coroutine.TERMINATED) { // When gets terminated
          that.addCount += 1;
  ...
  that.addPointsAsMarkersTogroup = function (points, callback) {
    ...
    that.points = points;
    for (i = 0; i  < that.numPoints; i += 1) {
      coroutine = that.add70MarkersTogroup(i, that.group);
      coroutine.addObserver('status', coroutineObserver);
    }
  };
}
